...
// Phase 8 最後收尾：檢查 Portal 是否已啟用
$portalEnabled = (int)db_query_one("SELECT customer_portal_enabled FROM settings WHERE id = 1")['customer_portal_enabled'] ?? 0;

if (!$portalEnabled) {
    $error = '客戶自助服務 Portal 目前已暫時停用。如有需要，請聯絡店舖。';
} elseif ($token) {
...
<body>
    ...
    async function loadPlans() {
        ...
        container.innerHTML = `<div class="text-center py-4"><div class="spinner-border spinner-border-sm text-primary" role="status"></div> <span class="small text-muted">載入中...</span></div>`;
                ...
                <div class="modal-content">
                  ...
                  <div class="modal-body">
                    ...
                    <div class="mb-3">
                      ...
                      <select class="form-select" id="portal-payment-method">
    ...
    async function submitPortalPayment(planId) {
        ...
        const methodInput = document.getElementById('portal-payment-method');
        const notesInput = document.getElementById('portal-payment-notes');
        const amount = parseFloat(amountInput.value);
        const methodId = methodInput ? parseInt(methodInput.value) : 0;
    ...
    async function loadPaymentMethods() {
        try {
            const res = await fetch('/api/payment_methods.php?action=list_active_public');
            const data = await res.json();
            if (data.success) {
                paymentMethods = data.data || [];
            }
        } catch (e) {
            paymentMethods = [{id: 1, name: '現金'}];
        }
    }
