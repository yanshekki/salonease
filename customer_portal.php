...
<body>
    ...
    function showRecordPayment(planId, suggestedAmount) {
        const amount = prompt('請輸入付款金額（HKD）：', suggestedAmount || '');
        if (!amount || isNaN(parseFloat(amount))) return;

        // Phase 8 後續會接真實記錄付款 API。目前先顯示提示。
        alert(`感謝！你已提交 HK$${parseFloat(amount).toFixed(0)} 的付款記錄。\n店舖會盡快確認並更新你的計劃進度。\n\n（完整記錄付款功能正在完善中）`);
        // TODO: 呼叫 Portal 專用記錄付款端點
    }
