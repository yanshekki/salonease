...
### 部署建議
1. 將專案目錄設為網站根目錄（或子目錄並正確設定 `.htaccess`）
2. 確保 `config.php` 已正確填寫資料庫連線資訊（**切勿上傳到 Git**）
3. 執行 `sql/schema.sql` 建立表格
4. （建議）執行 `sql/seeds.sql` 插入初始資料
5. 建議使用 HTTPS
6. 定期備份資料庫（尤其是 `sales`、`commissions`、`customer_packages` 表）

### 已知限制（v1.0 階段）
- 佣金目前仍以「開單人」為主，服務/零售佣金可透過 POS「指派員工」功能分拆
- 尚未支援多店面或分店架構
- 庫存扣減僅在結帳時處理，無先進先出或批次管理
- 報表目前以日期為主，尚未有圖表匯出或 PDF 功能
- 無前台客戶自助預約功能
