...
**Phase 3 目前真實進度**：約 77%（運維工具基礎 + Audit Log 查詢加強 + 統一匯出入口已完成，其餘項目陸續推進中）。

**目前進行中**：A148 - 統一匯出入口（用戶選擇 B）。這是 Phase 3 最後收尾 chunk，讓用戶能快速找到所有匯出功能。

**A148 定義（合理大小 chunk）**：
- 在 settings.php 加入「資料匯出中心」卡片
- 列出目前所有主要匯出功能（忠誠度 CSV、員工排行 CSV、Audit Log CSV、報表數據、庫存備份等）
- 提供一鍵快速連結或按鈕
- 簡單說明每個匯出的用途
- **完成定義**：設定頁可清楚看到所有匯出入口，一目了然
...
**A148 進行中**（用戶選擇 B）：統一匯出入口（在設定頁提供所有匯出功能快速入口）
