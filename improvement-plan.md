...
1. **報表強化與視覺化**
   ...
   - 加強日期範圍 + 多維度篩選體驗
   - 新增「庫存周轉率」與「缺貨趨勢」報表
   - **報表頁 JS 結構清理**（自 A108 開始）：修復 reportsApp() Alpine component 內 orphaned methods 問題，提升報表頁穩定性（A110 移動 loadSummary，A111 移動 loadPaymentBreakdown，A112 移動 loadTopProducts，A113 移動 loadTopServices）
