...
**Phase 3 目前真實進度**：約 65%（Sales Trend 完整區塊 A140~A142 已穩固，A143 開始處理庫存相關報表，其餘項目陸續推進中）。

**目前進行中**：A143 - 庫存周轉率 + 缺貨趨勢報表（用戶選擇 B）。這是 Phase 3 下一個有意義的報表 chunk，目標是讓管理層能看到庫存健康狀況與趨勢。

**A143 定義（合理大小 chunk）**：
- 後端：在 api/reports.php 新增兩個 action
  - `inventory_turnover`：計算每個產品的周轉率（銷售數量 / 平均庫存）、周轉天數
  - `stockout_trend`：每日/每週缺貨趨勢（低於門檻產品數量變化）
- 支援 from/to 日期範圍
- 前端：在 reports.php 新增「庫存分析」區塊（兩個圖表 + 表格）
  - 庫存周轉率排行（長條圖 + 表格）
  - 缺貨趨勢線圖
- 加入 loading、空資料提示、日期篩選
- 簡單 CSV 匯出
- **完成定義**：報表頁可顯示庫存周轉率與缺貨趨勢，數據來自真實 sales + products 表，操作流暢
...
**A143 進行中**（用戶選擇 B）：建立庫存周轉率 + 缺貨趨勢報表（後端 + 前端完整區塊）
