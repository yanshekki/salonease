...
**Phase 3 目前真實進度**：約 79%（運維工具 + Audit Log + 統一匯出 + 打印優化已開始，其餘項目陸續推進中）。

**目前進行中**：A149 - 打印優化（用戶選擇 A）。這是 Phase 3 最後收尾 chunk，改善收據與報表打印體驗。

**A149 定義（合理大小 chunk）**：
- 改善熱感紙收據排版（對齊、字體大小、間距）
- 改善 A4 打印排版一致性
- 加入可配置選項（是否打印 logo、稅項、員工姓名等，可從設定頁控制）
- 統一樣式（print.css 強化）
- 確保 POS 打印與 A4 打印風格一致
- **完成定義**：打印功能排版更清晰美觀，設定可控
...
**A149 進行中**（用戶選擇 A）：打印優化（熱感紙 + A4 排版改善 + 可配置選項 + 統一樣式）
